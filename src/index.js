  ...
  listIcon.addEventListener("click", (e) => {
    ...
    todoMainAll.forEach((items) => {
      ...
      if (!items.classList.contains(`onClickTodoMainShrink`)) {
        const todoMinimizeBtnAll =
          document.querySelectorAll(".todoMinimizeBtn");
...
function minimizeTodoDetailsFnc(e) {
  e.currentTarget.parentNode.parentElement.parentElement.classList.toggle(
  ...
  e.currentTarget.parentNode.parentElement.childNodes[1].classList.toggle(
    "onClickHideToDoDetails"
  ); // Selects todoDescription
  e.currentTarget.parentNode.parentElement.childNodes[2].classList.toggle(
    "onClickTodoDate"
  ); // Selects todoDate
  e.currentTarget.parentNode.parentElement.childNodes[3].classList.toggle(
    "onClickHideToDoDetails"
  ); // Selects todoStatus

  e.currentTarget.parentNode.childNodes[0].classList.toggle(
    "onClickHideToDoDetails"
  ); // Selects deleteBtn
  e.currentTarget.parentNode.childNodes[1].classList.toggle(
    "onClickHideToDoDetails"
  ); // Selects editBtn
  console.log(e.currentTarget.parentNode.parentElement);

  if (
    e.currentTarget.parentNode.parentElement.parentElement.classList.contains(
      `onClickTodoMainShrink`
    )
  ) {
...
function enableEditBtn() {
  ...
  todoEditBtnAll.forEach((isTodoEditClicked) => {
    ...
    isTodoEditClicked.parentElement.parentElement.parentElement.classList.remove(
      "onClickEditTodoMainStyle"
    ); // removes todoMain Border and BGColor
...
function saveTodoEachAfterEdit(e) {
  const saveBtnEditVariable =
    e.currentTarget.parentNode.parentElement.parentElement.childNodes[1]
      .childNodes[1].childNodes[9].childNodes[1]; // selects saveBtnEdit
  saveBtnEditVariable.addEventListener("click", (event) => {
    if (
      ...
      const dateBtnIndianFormat = dateBtnEdit.value
        .split("-")
        .reverse()
        .join("-");
      addTodoDiv.style.display = "flex";
      editTodo.style.display = "none";
      event.currentTarget.parentNode.parentElement.parentElement.parentElement.parentElement.style.border =
        "none"; // selects details with respect to editBtn
      event.currentTarget.parentNode.parentElement.parentElement.parentElement.parentElement.style.backgroundColor =
        "rgb(230, 230, 230)"; // selects details with respect to editBtn
      event.currentTarget.parentNode.parentElement.parentElement.parentElement.childNodes[0].style.display =
        "grid"; // selects details
      ...
      event.currentTarget.parentNode.parentElement.parentElement.parentElement.childNodes[0].childNodes[2].textContent =
        dateBtnIndianFormat;
...
function editTodoEach() {
  ...
  todoEditBtnAll.forEach((items, index) => {
    ...
    items.addEventListener("click", (e) => {
      e.currentTarget.parentNode.parentElement.parentElement.classList.add(
        "onClickEditTodoMainStyle"
      );
...
function predefinedTodoInput() {
  const listItemTwo = new TodoClass(
    "Lionel Messi",
    "The GOAT",
    "24-06-1987",
    "High"
